...
export default function SiteShell({ children }) {
  ...
  // auto-pause when header is meaningfully visible
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        const e = entries[0];
        if (!e) return;
        setPaused(e.isIntersecting && e.intersectionRatio >= 0.65);
      },
...
.tm-status:hover .tm-status-inner{ animation-play-state:paused; }
        ...
        /* Status items (enterprise, no pills) */
        .tm-status-item{
          appearance:none;
          border:0;
        ...
        /* YOUR HERO FORM STYLES (left as-is) */
        .tru-hero-formcard{
          border-radius:18px;
...
.tru-hero2-accent{
  color:#0b1220;
  text-shadow:0 18px 60px rgba(15,23,42,0.10);
}
...
@media (max-width: 520px){
  ...
  .tru-row2{ grid-template-columns:1fr; }
